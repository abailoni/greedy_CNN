...
class tune_lrn_rate(tune_hyperparams):
    def fit_model(self, param_values, model_name):
        ...
        convSoftmax_params['eval_size'] = prop
        ...
        # from mod_nolearn.segmentFcts import  pixel_accuracy_np, compute_mean_IoU_logRegr
        ...
        # pred_train_B, pred_val_B = greedy_routine.convSoftmax[net_name].net.predict(X[:size_train]), greedy_routine.convSoftmax[net_name].net.predict(data_X_val[:30])
        ...
        # print "After IoU:"
        # print compute_mean_IoU_logRegr(pred_train_B,y[:size_train]), compute_mean_IoU_logRegr(pred_val_B,data_y_val[:200])
        ...
        results = {
            ...
            'val pixelAcc': greedy_routine.convSoftmax[net_name].net.train_history_[best]['valid acc'],
            # 'Train IoU': greedy_routine.convSoftmax[net_name].net.train_history_[best]['Train IoU'],
            # 'Valid IoU': greedy_routine.convSoftmax[net_name].net.train_history_[best]['Valid IoU'],
        }
        return results
...
first = tune_lrn_rate(
    (
        ('size_data', range(200,1001,100), None, None, None),
    ),
