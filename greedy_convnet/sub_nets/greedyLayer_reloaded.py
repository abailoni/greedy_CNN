...
class greedyLayer_reload(object):
    ...
    def __init__(
        ...
        Descriptions of the inputs:
            ...
            - fixed_input_layers: nolearn-dictionary with previous fixed layers
            - layers_info: dictionary containing infos about all layers (e.g.
                output_shape, req, trained_greedily, trained, type...)
            - name_trained_layer
            - layer_kwargs: all the parameters passed to the selected layer
            - list_boost_filters: list with number of filters for each perceptron
                (e.g. [5,5,5,5,5,3])
            - FULLNET_kwargs: arguments passed to the main greedyNet during initialization (num_classes, batch_size, etc...)
            - greedy_kwargs: parameters passed to the fit function plus some
        ...
        # There are other parameters not used:
