...
class greedyLayer_reload(object):
    ...
    def __init__(
        ...
        Descriptions of the inputs:
            ...
            - fixed_input_layers:
                nolearn-dictionary with previous layers that won't be trained

            - fixed_weights:
                weights for the fixed layers

            - layers_info:
                dictionary containing infos about all layers (e.g.
                output_shape, req, trained_greedily, trained, type...)

            - name_trained_layer (mainly for log and to check infos)

            - layer_kwargs:
                all the parameters passed to the selected layer in the original
                nolearn network

            - list_boost_filters:
                list with number of filters for each perceptron
                (e.g. [5,5,5,5,5,3])

            - FULLNET_kwargs:
                arguments passed to the main greedyNet during initialization.
                They hold for every trained layer (e.g. num_classes, batch_size...)

            - greedy_kwargs:
                parameters passed to the fit function plus some
                automatically generated by init_greedyLayer (e.g. logs_path...).
                Usually here there are parameters like learning_rate, etc...
        '''
