...
# import lasagne
from lasagne.layers import InputLayer, Conv2DLayer, MaxPool2DLayer, ConcatLayer, Upscale2DLayer, TransposedConv2DLayer
...
def define_network(input_var, input_size=(572,572),
    ...
    def contraction(depth, deepest):
        ...
        incoming = 'pool{}'.format(depth-1)
        net['conv{}_1'.format(depth)] = (Conv2DLayer, {'incoming': incoming,
                                    'name': 'conv{}_1'.format(depth),
                                    'num_filters':n_filters, 'filter_size':3, 'pad':'same',
                                    'W': HeNormal(gain='relu'),
                                    'nonlinearity': nonlinearity} )
        # Input layer does not have a name:
        if depth == 0:
            net['conv{}_1'.format(depth)][1].pop('incoming')

        net['conv{}_2'.format(depth)] = (Conv2DLayer, { 'incoming': 'conv{}_1'.format(depth),
                                    'name': 'conv{}_2'.format(depth),
                                    'num_filters':n_filters, 'filter_size':3, 'pad':'same',
                                    'W':HeNormal(gain='relu'),
                                    'nonlinearity': nonlinearity} )

        if not deepest:
            net['pool{}'.format(depth)] = (MaxPool2DLayer, {'incoming': 'conv{}_2'.format(depth), 'name':'pool{}'.format(depth), 'pool_size': 2, 'stride':2} )

    def expansion(depth, deepest):
        n_filters = _num_filters_for_depth(depth, branching_factor)

        incoming = 'conv{}_2'.format(depth+1) if deepest else '_conv{}_2'.format(depth+1)


        # OLD IMPLEMENTATION WITHOUT TRANS. CONVOLUTION:
        # upscaling = Upscale2DLayer(incoming, 4)
        # net['upconv{}'.format(depth)] = (Conv2DLayer, {'incoming': upscaling,
        #                                 'name': 'upconv{}'.format(depth),
        #                                 'num_filters':n_filters, 'filter_size':2, 'stride':2,
        #                                 'W':HeNormal(gain='relu'),
        #                                 'nonlinearity': nonlinearity} )
        # upscaling = Upscale2DLayer(incoming, 4)
        net['transConv{}'.format(depth)] = (TransposedConv2DLayer, {'incoming': incoming,
                                        'name': 'transConv{}'.format(depth),
                                        'num_filters':n_filters, 'filter_size':2, 'stride':2,
                                        'W':HeNormal(gain='relu'),
                                        'nonlinearity': nonlinearity} )

        net['bridge{}'.format(depth)] = (ConcatLayer, {'incomings': [
                                        'transConv{}'.format(depth),
                                        'conv{}_2'.format(depth)],
                                        'name': 'bridge{}'.format(depth),
                                        'axis':1, 'cropping':[None, None, 'center', 'center']})

        net['_conv{}_1'.format(depth)] = (Conv2DLayer, {'incoming': 'bridge{}'.format(depth),
                                        'name': '_conv{}_1'.format(depth),
                                        'num_filters': n_filters, 'filter_size':3, 'pad':'same',
                                        'W': HeNormal(gain='relu'),
                                        'nonlinearity': nonlinearity})

        net['_conv{}_2'.format(depth)] = (Conv2DLayer, {'incoming': '_conv{}_1'.format(depth),
                                        'name': '_conv{}_2'.format(depth),
                                        'num_filters': n_filters, 'filter_size':3, 'pad':'same',
                                        'W':HeNormal(gain='relu'),
                                        'nonlinearity': nonlinearity} )
    ...
    # Output layer
    net['out'] = (Conv2DLayer, {'incoming': '_conv0_2', 'name': 'out', 'num_filters':num_classes, 'filter_size':(1,1), 'pad': 'same',
                                    'nonlinearity': None} )
